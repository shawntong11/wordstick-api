...
function getUserId() {
  try {
    // Priority 1: URL param ?uid=xxx (passed by plugin when opening review page)
    const urlUid = new URLSearchParams(location.search).get('uid');
    if (urlUid) {
      localStorage.setItem('ws_user_id', urlUid); // sync to localStorage
      return urlUid;
    }
...
async function init() {
  ...
  // Load tree from API — includes user's saved words merged into categories
  try {
