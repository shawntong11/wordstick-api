...
async function init() {
  ...
  // Load static wordhub tree + user words separately, merge in frontend
  try {
    const [treeRes, userRes] = await Promise.all([
      fetch('/wordhub_tree.json'),
      fetch(API_URL + '/api/words?type=user', { headers: { 'X-User-ID': getUserId() } })
    ]);
    tree = await treeRes.json();

    // Merge user words into tree
    ...
    if (userWords.length > 0) {
      ...
      // Collect all existing words to avoid duplicates
      ...
      for (const w of userWords) {
        ...
        // Tree is 3-level: L1 > L2 > L3 > [words]
        // Insert user words as: L1['📌 我保存的']['我的单词'] = [words]
        const L2key = '📌 我保存的';
        const L3key = '我的单词';
