...
// ── Init ──────────────────────────────────────────────────────────────
async function init() {
  // Resolve userId first (writes URL param ?uid= into localStorage)
  const resolvedUserId = getUserId();

  // Load WordStick words
  wsWords = await loadWSWords();

  // Step 1: load static tree (critical)
  try {
    const treeRes = await fetch('/wordhub_tree.json');
  ...
  // Step 2: merge user words (optional, never overwrites tree on failure)
  try {
    const userRes = await fetch(API_URL + '/api/words?type=user', {
      headers: { 'X-User-ID': resolvedUserId }
    });
    const userData = await userRes.json();
    const userWords = userData.words || [];
    if (userWords.length > 0 && Object.keys(tree).length > 0) {
      const CAT_KEY_MAP = {
        general:'0.通用词', academic:'1.校园', workplace:'2.工作',
        technology:'3.技术', stem:'4.数理化', geography:'5.地理',
        life:'7.社会', arts:'8.艺术', politics:'10.政治', economics:'11.经济'
      };
      const existing = new Set();
      function collect(node) {
        if (Array.isArray(node)) node.forEach(w => existing.add((w.w||'').toLowerCase()));
        else if (node && typeof node === 'object') Object.values(node).forEach(collect);
      }
      collect(tree);
      for (const w of userWords) {
        if (existing.has(w.word.toLowerCase())) continue;
        const topKey = CAT_KEY_MAP[w.category] || '0.通用词';
        if (!tree[topKey]) continue;
        const L2key = '📌 我保存的';
        const L3key = '我的单词';
        if (!tree[topKey][L2key]) tree[topKey][L2key] = {};
        if (!tree[topKey][L2key][L3key]) tree[topKey][L2key][L3key] = [];
        tree[topKey][L2key][L3key].push({
          w: w.word, i: w.phonetic||'', img: w.image_url||'',
          zh: w.primary_zh||'', source: 'wordstick'
        });
      }
    }
  } catch(e) {
    console.warn('User words load failed (non-critical):', e);
  }
  window.wordTree = tree;
...
async function loadWSWords() {
  // Web version: load from cloud API
